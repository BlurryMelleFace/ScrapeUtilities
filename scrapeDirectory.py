...
def combine_file_contents(dir_path, output_folder):
    ...
    current_output_file = os.path.join(
        output_folder, f"{os.path.basename(output_folder)}_part{file_counter}.txt"
    )
...
def add_to_context_menu():
    ...
    icon_path = os.path.join(
        os.path.dirname(script_path), "scrape_icon.ico"
    )  # Path to the icon
    ...
    try:
        ...
        with reg.CreateKey(reg.HKEY_CLASSES_ROOT, command_path) as key:
            reg.SetValue(
                key, "", reg.REG_SZ, f'"{app_path}" "{script_path}" "%1"'
            )  # Explicitly run script
...
def remove_from_context_menu():
    ...
    try:
        with reg.OpenKey(
            reg.HKEY_CLASSES_ROOT, reg_path + r"\command", 0, reg.KEY_WRITE
        ) as command_key:
            reg.DeleteKey(reg.HKEY_CLASSES_ROOT, reg_path + r"\command")
            print("Deleted 'command' subkey.")
        with reg.OpenKey(
            reg.HKEY_CLASSES_ROOT, reg_path, 0, reg.KEY_WRITE
        ) as runmyapp_key:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if action == "add":
            ...
            print(
                f"Invalid argument: {sys.argv[1]}. Use 'add', 'remove', or pass a valid directory path."
            )
            sys.exit()
    else:
    ...
    if "directory_path" in locals() and directory_path:
        ...
        output_folder = os.path.join(
            os.path.expanduser("~\ScrapeUtility\ScrapedDirectories"),
            os.path.basename(directory_path.rstrip(os.sep)),
        )
        os.makedirs(output_folder, exist_ok=True)
